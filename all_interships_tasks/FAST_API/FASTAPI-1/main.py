...
products = [
    ...
    {"id": 4, "name": "USB-C Cable", "price": 199, "category": "Electronics", "in_stock": True},
...
@app.get("/")
def root():
    return {"message": "FastAPI Day 1 Assignment is running. Visit /docs"}
...
def get_by_category(category_name: str):
    ...
    result = [p for p in products if str(p.get("category", "")).lower() == needle]
    if not result:
...
def search_products(keyword: str):
    ...
    results = [p for p in products if k in str(p.get("name", "")).lower()]
    if not results:
